...
module.exports = {
    ...
    getAllClients: function(req, res) {
      ...
      getPastAppointments: function(req, res) {
        sequelize.query(`
        SELECT cc_appointments.appt_id, 
        cc_appointments.date, 
        cc_appointments.service_type, 
        cc_appointments.notes, 
        cc_users.first_name, 
        cc_users.last_name 
        FROM 
        cc_appointments 
      ...
      WHERE 
        cc_appointments.approved = true AND 
        cc_appointments.completed = true 
        `).then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => res.status(500).send(err));
      },
